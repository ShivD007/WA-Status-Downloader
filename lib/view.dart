...
class View extends StatefulWidget {
  ...
  const View({Key key,
  // this.list, 
   this.index, this.flag}) : super(key: key);
...
positioned = widget.index;
 ...
 tabindex = Provider.of<TabIndexProvider>(context, listen:  false);
  ...
  Widget build(BuildContext context) {
     ...
     widget.flag == 0 ? _list = Provider.of<ImageVideoProviders>(context).getlist : _list = Provider.of<ImageVideoProviders>(context).getvideoList ;
     
    return WillPopScope(
      onWillPop: () async {
        print(".......................................................vhalaaaaaaaaaaaaaaaa...........................................");
        ...
        body: SafeArea(
          child: Stack(
            children: <Widget>[
              PageView.builder(
                ...
                itemBuilder: (context , positioned){
                  ...
                  child: Center(
                    child: widget.flag == 0
                          ? Hero(tag: _list[positioned], child: Image.file(File(_list[positioned])))
            ...
            child: Padding(
              ...
              child: Text(positioned.toString(),style:TextStyle(color :Colors.white),)
            ),),

            Align(alignment: Alignment.topRight, 
            child: Padding(
              padding: const EdgeInsets.all(20),
              child: IconButton( icon :Icon(Icons.share,color: Colors.white,), onPressed: () async{ 
                final ByteData bytes = await rootBundle.load(_list[positioned]);
                final tempList = _list[positioned].toString().split("/");
                final filename = tempList[tempList.length -1];
                print(filename +"ggggggggggggggggggggggggggggggg");

await Share.file('esys image', filename, bytes.buffer.asUint8List(), "${widget.flag == 0 ? "image/*" : "video/*"}", text: 'Send by WA Status Downloader. To share mre status like this go and downlaod');},)
            ),),
         ...
         if (value == false){
           return Align(alignment: Alignment.topRight + Alignment(-0.3, 0), 
            child: Padding(
              padding: const EdgeInsets.all(20),
              child: IconButton( icon :Icon(Icons.file_download,color: Colors.white,), onPressed: () async{ await _saveFile(context);},)
            )); 
           ...
           return Align(alignment: Alignment.topRight + Alignment(-0.3, 0), 
            child: Padding(
              ...
              child: IconButton( icon :Icon(Icons.delete,color: Colors.white,), onPressed: (){ 
                Provider.of<ImageVideoProviders>(context,listen: false).delete(_list[positioned], widget.flag);
                setState(() {
